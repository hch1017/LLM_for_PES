...
def generate_predictions(
    ...
    **kwargs
    ...
    completions_list = []
    complete = lambda x: completion_fn(input_str=x, task_name=task_name, steps=steps, settings=settings, num_samples=num_samples, temp=temp)
...
def get_llmtime_predictions_data(train, test, task_name, model, settings, num_samples=10, temp=0.7, alpha=0.95, beta=0.3, basic=False, parallel=True, **kwargs):
    """
    ...
    if num_samples > 0:
        preds, completions_list, input_strs = generate_predictions(completion_fn, task_name, input_strs, steps, settings, scalers,
