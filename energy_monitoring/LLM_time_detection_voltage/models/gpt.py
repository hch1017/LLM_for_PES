...
def gpt_completion_fn(instances, model, task_name, input_str, steps, settings, num_samples, temp):
    ...
    if model in ['gpt-3.5-turbo','gpt-4', 'gpt-4-turbo']:
        ...
        if task_name == 'voltage_anomaly_detection':
            chatgpt_sys_message = "You are a helpful assistant that performs {}. The user will provide a sequence and you will predict if voltage anomaly exists in this sequence by [0, 1, 2, 3], where 0 means normal, 1 means voltage distortion, 2 means voltage sag, and 3 means voltage impulse. The sequence is represented by decimal strings separated by commas.".format(task_name)
            # extra_input = "Please give your answer, i.e., just a number in [0,1,2,3], without producing any additional text. Do not say anything like 'the quality condition of the sequence is', just return the number. Number:\n"
            extra_input = "Please give your answer, i.e., a number in [0,1,2,3], and explain it.\n"
