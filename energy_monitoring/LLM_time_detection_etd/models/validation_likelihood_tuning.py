...
def evaluate_hyper(hyper, train_minus_val, val, task_name, get_predictions_fn):
    ...
    return get_predictions_fn(train_minus_val, val, task_name, **hyper, num_samples=0)['NLL/D']


def get_autotuned_predictions_data(train, test, instances, task_name, hypers, num_samples, get_predictions_fn, verbose=False, parallel=True, n_train=None, n_val=None):
    """
    ...
    if len(hypers) > 1:
        ...
        def eval_hyper(hyper):
            try:
                return hyper, evaluate_hyper(hyper, train_minus_val, val, task_name, get_predictions_fn)
            except ValueError:
    ...
    out = get_predictions_fn(train, test, instances, task_name, **best_hyper, num_samples=num_samples, n_train=n_train)
