...
def generate_predictions(
    ...
    **kwargs
    ...
    completions_list = []
    complete = lambda x: completion_fn(instances=instances, input_str=x, task_name=task_name, steps=steps, settings=settings, num_samples=num_samples, temp=temp)
    ...
    # print('cl:', completions_list[0][0])
    # print('cl shape:', np.array(completions_list).shape)
    # print('input:', input_strs[0])
    # print('input shape:', np.array(input_strs).shape)
    return completions_list, input_strs

def get_llmtime_predictions_data(train, test, instances, task_name, model, settings, num_samples=10, temp=0.7, alpha=0.95, beta=0.3, basic=False, parallel=True, **kwargs):
    """
    ...
    # 最后把instances做成一个list，装的是每个shot的样例
    instances_shot_list = []
    scalers_instance = get_scaler(np.array(instances.iloc[:,:-1]).reshape(1,-1), alpha=alpha, beta=beta, basic=basic)
    output = np.array(instances['output'])
    for i in range(len(instances)):
        input_arrs_inst = np.array(instances.iloc[i, :-1])
        transformed_input_arrs_inst = scalers_instance.transform(input_arrs_inst)
        input_strs_inst = serialize_arr(transformed_input_arrs_inst, settings)
        input_arrs_inst, input_strs_inst = zip(*[truncate_input(input_arrs_inst, input_strs_inst, settings, model)])
        # print(input_arrs_inst)
        
        shot = "Sequence: " + input_strs_inst[0] + ". True label: " + str(output[i]) + '\n ' 
        instances_shot_list.append(shot)
        # print(instances_shot_list[0])
        # sys.exit()
    ...
    if num_samples > 0:
        completions_list, input_strs = generate_predictions(instances_shot_list, completion_fn, task_name, input_strs, steps, settings, scalers,
    ...
    if nll_fn is not None:
        BPDs = [nll_fn(input_arr=input_arrs[i], task_name=task_name, target_arr=test[i].values, settings=settings, transform=scalers[i].transform, count_seps=True, temp=temp, steps=steps, num_samples=num_samples) for i in range(len(train))]
