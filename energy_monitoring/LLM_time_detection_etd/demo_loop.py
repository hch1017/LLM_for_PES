...
task_names = ['etd']
task_full_names = ['energy theft detection']
...
for task_name, task_full_name in zip(task_names, task_full_names):
    if task_name == 'power_quality_assessment':
        df = pd.read_csv('D:\Monash\LLM_time_detection\datasets\PowerQualityDistributionDataset1.csv')
        df = df.drop(['Unnamed: 0'], axis=1)
    elif task_name == 'line_fault':
        df = pd.read_csv('D:\Monash\LLM_time_detection\datasets\Fault_line_detection/train_dataset.csv')
        df = df.iloc[:, 4:] # 这个不是时序数据
        df.rename(columns={'Fault_Type':'output'}, inplace=True)
        df['output'] = (df['output'] > 0).astype(int)
    elif task_name == 'etd':
        import numpy as np
        from collections import defaultdict

        df = pd.read_csv('D:\Monash\LLM_time_detection_etd/datasets/etd_for_llm.csv')
        df = df[df['ID']==1000]
        df = df.drop(columns=['ID', 'day'])
        df.rename(columns={'theft': 'output'}, inplace=True)

    elif task_name == 'turbine_fault':
        df = pd.read_csv('D:\Monash\LLM_time_detection\datasets\CARE_To_Compare_Data\CARE_To_Compare\Wind_Farm_A\Wind_Farm_A\datasets/0.csv', sep=';')
        cols = [col for col in df.columns if col != 'status_type_id']
        cols.append('status_type_id')
        df = df[cols]
        df = df.iloc[:,4:]
        df.rename(columns={'status_type_id':'output'}, inplace=True)
        df['output'] = (df['output'] > 0).astype(int)
        
    # 查看所有的unique label，这里是0和1
    values_to_extract = df.iloc[:,-1].unique().tolist()
    
    # 每个label的测试样本数 %20
    samples_per_values = [1,1] 
    ...
    path = 'D:\Monash\LLM_time_detection_etd/{}_results/'.format(task_name)
    if not os.path.exists(path):
        os.makedirs(path)
    np.save('D:\Monash\LLM_time_detection_etd/{}_results/output_{}_{}_{}.npy'.format(task_name, task_name, instances_per_value, samples_per_values[0]), np.array(output_list))
    np.save('D:\Monash\LLM_time_detection_etd/{}_results/label_{}_{}_{}.npy'.format(task_name, task_name, instances_per_value, samples_per_values[0]), np.array(label_list))
